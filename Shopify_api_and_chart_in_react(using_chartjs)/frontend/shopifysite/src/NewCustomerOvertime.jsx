...
const NewCustomersOverTimeChart = ({ salesData, timePeriod }) => {
  // Check if salesData is defined and is an array
  if (!salesData || !Array.isArray(salesData)) {
    return <p>No new customers data available</p>;
  }
  ...
  const dataValues = salesData.map(item => item.newCustomers);
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `New Customers Over Time(${timePeriod.charAt(0).toUpperCase() + timePeriod.slice(1)})`,
      },
    },
    scales: {
      x: {
