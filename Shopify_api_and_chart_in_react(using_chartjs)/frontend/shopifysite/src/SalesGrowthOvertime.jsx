...
const SalesGrowthOverTimeChart = ({ salesData, timePeriod }) => {
  ...
  const dataValues = salesData.map(item => item.growthRate || 0);
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Sales Growth Rate Over Time (${timePeriod.charAt(0).toUpperCase() + timePeriod.slice(1)})`,
      },
    },
    scales: {
      x: {
...
export default SalesGrowthOverTimeChart;
